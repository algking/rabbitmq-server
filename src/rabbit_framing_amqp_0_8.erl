...
-type(amqp_method_field_name() ::
       ( prefetch_size | operation | prefetch_count | consumer_tag
       | global | redelivered | ticket | integer_3
       | queue | consumer_tag | no_local | no_ack
       | exclusive | nowait | no_ack | consumer_tag
       | consumer_tag | exclusive | nowait | message_count
       | dtx_identifier | consumer_tag | integer_4 | nowait
       | identifier | content_size | staged_size | ticket
       | exchange | routing_key | mandatory | immediate
       | cluster_id | identifier | integer_1 | integer_2
       | reply_code | reply_text | exchange | routing_key
       | result | consumer_tag | delivery_tag | redelivered
       | exchange | routing_key | identifier | operation
       | string_1 | result | delivery_tag | delivery_tag
       | multiple | table | integer_op | delivery_tag
       | string_2 | requeue | multiple | integer_result
       | string_result | nowait | content_checksum | prefetch_size
       | prefetch_count | consume_rate | global | ticket
       | consumer_tag | queue | consumer_tag | requeue
       | no_local | exclusive | nowait | consumer_tag
       | consumer_tag | nowait | consumer_tag | ticket
       | exchange | routing_key | requeue | mandatory
       | immediate | exchange | reply_code | reply_text
       | exchange | routing_key | string_op | consumer_tag
       | delivery_tag | exchange | queue | immediate
       | ticket | version_major | queue | nowait
       | message_count | queue | ticket | queue
       | if_unused | if_empty | nowait | exchange
       | message_count | ticket | routing_key | version_minor
       | server_properties | reply_text | mechanisms | locales
       | client_properties | arguments | mechanism | response
       | exchange | locale | challenge | prefetch_size
       | response | global | routing_key | channel_max
       | frame_max | heartbeat | channel_max | frame_max
       | heartbeat | no_local | virtual_host | capabilities
       | insist | known_hosts | consumer_tag | host
       | known_hosts | consumer_tag | reply_code | consumer_tag
       | reply_text | class_id | method_id | meta_data
       | ticket | delivery_tag | out_of_band | routing_key
       | mandatory | active | active | redelivered
       | reply_code | reply_code | reply_text | details
       | reply_code | reply_text | class_id | method_id
       | exchange | routing_key | realm | exclusive
       | passive | active | prefetch_count | write
       | read | delivery_tag | ticket | exchange
       | routing_key | ticket | exchange | type
       | passive | durable | auto_delete | internal
       | nowait | ticket | arguments | delivery_tag
       | ticket | exchange | if_unused | queue
       | nowait | requeue | ticket | queue
       | no_ack | passive | durable | exclusive
       | auto_delete | nowait | arguments | queue
       | message_count | consumer_count | ticket | ticket
       | queue | exchange | routing_key | queue
       | nowait | arguments )).
