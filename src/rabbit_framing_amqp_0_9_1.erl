...
-type(amqp_method_field_name() ::
       ( consumer_tag | delivery_tag | redelivered | prefetch_count
       | exchange | ticket | queue | no_ack
       | no_local | delivery_tag | redelivered | exclusive
       | exchange | routing_key | arguments | message_count
       | passive | durable | auto_delete | internal
       | nowait | arguments | message_count | routing_key
       | ticket | exchange | if_unused | nowait
       | ticket | destination | source | routing_key
       | nowait | arguments | consumer_tag | cluster_id
       | delivery_tag | ticket | destination | ticket
       | source | routing_key | nowait | nowait
       | arguments | multiple | delivery_tag | requeue
       | requeue | ticket | queue | if_unused
       | passive | durable | exclusive | auto_delete
       | nowait | arguments | if_empty | requeue
       | queue | message_count | consumer_count | nowait
       | ticket | queue | exchange | routing_key
       | nowait | arguments | ticket | queue
       | nowait | ticket | version_major | version_minor
       | server_properties | message_count | mechanisms | locales
       | exchange | queue | client_properties | mechanism
       | response | locale | routing_key | challenge
       | response | ticket | channel_max | frame_max
       | mandatory | heartbeat | arguments | channel_max
       | frame_max | nowait | heartbeat | virtual_host
       | capabilities | requeue | insist | prefetch_size
       | queue | known_hosts | global | reply_code
       | reply_text | class_id | method_id | exchange
       | ticket | queue | consumer_tag | out_of_band
       | no_ack | channel_id | routing_key | nowait
       | active | active | consumer_tag | reply_code
       | reply_text | class_id | reply_text | method_id
       | consumer_tag | realm | exclusive | exchange
       | passive | active | delivery_tag | write
       | read | immediate | ticket | routing_key
       | reply_code | multiple | ticket | exchange
       | type )).
